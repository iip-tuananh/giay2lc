...
<div ng-cloak>
    ...
    {{-- Form tạo mới --}}
    @include('admin.group_tag.create')
    @include('admin.group_tag.edit')
</div>
@endsection

@section('script')
@include('admin.group_tag.TagGroup')
<script>
    let datatable = new DATATABLE('table-list', {
        ajax: {
            url: '{!! route('tagGroups.searchData') !!}',
    ...
    app.controller('Attribute', function ($rootScope, $scope, $http) {
        ...
        $('#table-list').on('click', '.edit', function () {
            ...
            $.ajax({
                type: 'GET',
                url: "/admin/tag-groups/" + $scope.data.id + "/getDataForEdit",
                headers: {
                ...
                success: function(response) {
                    if (response.success) {
                        if(response.data.tags.length > 0) {
                            swal({
                                title: "Xác nhận!",
                                text: `Nhóm này đã được gán thẻ. Đồng ý xóa?`,
                                type: "warning",
                            ...
                            swal({
                                title: "Xác nhận xóa!",
                                text: "Bạn chắc chắn muốn xóa nhóm thẻ này?",
                                type: "warning",
                                showCancelButton: true,
                                confirmButtonClass: "btn-danger",
                                confirmButtonText: "Xác nhận",
